...
var indexGenerator = function(max) {  //function to make random number
  return Math.floor(Math.random() * Math.max(max))
}


var startupPitch =function() {
...
var myNewStartUp = function() { //function to create startup string
  return `${thisArray[indexGenerator(thisArray.length)]} for ${thatArray[indexGenerator(thatArray.length)]}`
}
